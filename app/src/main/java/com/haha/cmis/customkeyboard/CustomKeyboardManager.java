package com.haha.cmis.customkeyboard;
...
public class CustomKeyboardManager implements OnFocusChangeListener, View.OnClickListener {
    ...
    private boolean isKeyboardShowed = false; //是否已经显示

    public CustomKeyboardManager(@NonNull Activity activity) {
        mContext = activity;
    ...
    public void attachTo(@NonNull EditText editText, @NonNull CustomBaseKeyboard keyboard) {
    ...
    private CustomBaseKeyboard getKeyboard(@NonNull View view) {
     ...
     * @return 移动偏移量
     */
    private int getMoveHeight(@NonNull View view) {
        Rect rect = new Rect();
    ...
    public void onClick(View v) {
        ...
        if (v instanceof EditText) {
            EditText attachEditText = (EditText) v;
            if (!isKeyboardShowed) showSoftKeyboard(attachEditText);
        }
    }

    public void showSoftKeyboard(@NonNull EditText view) {
//        if (getSoftKeyboardStatus()) return;
        ...
        if (isKeyboardShowed) mRootView.removeView(mKeyboardViewContainer);
        //将键盘布局加入到根布局中.
    ...
    public void hideSoftKeyboard(@NonNull EditText et) {
        int moveHeight = 0;
        Object tag = et.getTag(R.id.keyboard_view_move_height);
        if (null != tag) moveHeight = (int) tag;
        if (moveHeight > 0) { //复原屏幕
            mRootView.getChildAt(0).scrollBy(0, -1 * moveHeight);
            et.setTag(R.id.keyboard_view_move_height, 0);
        }
    ...
    public boolean getSoftKeyboardStatus() {
        LogUtils.d("getSoftKeyboardStatus: " + isKeyboardShowed);
     ...
     * @param editText
     */
    public static void hideSystemSoftKeyboard(@NonNull EditText editText) {
